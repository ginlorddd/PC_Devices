...
namespace DM_OHD.DTO
{
    public class DieMasterDTO
    {
        ...
        {
            return DBUtils.GetData("SELECT ID, DIE_NO, DIE_NAME, TOTAL_CAVITY FROM DIE_MST ORDER BY DIE_NO, DIE_NAME, ID");
        }
        ...
        {
            DBUtils.Exec("DELETE FROM DIE_MST WHERE ID=@ID", new SqlParameter("@ID", id));
        }
