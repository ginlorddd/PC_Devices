...
(
    ...
    REPORT_FORM_CODE VARCHAR(100)
);

INSERT INTO @REQ(MANAGEMENT_NO, JIG_NAME, JIG_TYPE_CODE, JIG_TYPE, JIG_SIZE, USE_PRODUCT, PURPOSE_USE, LOCATION_CODE, FACTORY, DEPARTMENT, CHECK_FREQUENCY, LAST_CHECK_DATE, REPORT_FORM_CODE)
SELECT
    R.MANAGEMENT_NO,
    R.JIG_NAME,
    R.JIG_TYPE_CODE,
...
WHEN MATCHED THEN
    UPDATE SET
        ...
        T.REPORT_FORM_CODE = S.REPORT_FORM_CODE,
        T.IS_ACTIVE = 1,
        T.UPDATED_AT = GETDATE()
WHEN NOT MATCHED THEN
    INSERT (CONTROL_NO, JIG_NAME, JIG_TYPE, JIG_TYPE_CODE, JIG_SIZE, USE_PRODUCT, PURPOSE_USE, LOCATION_CODE, FACTORY, LAST_CHECK_DATE, NEXT_CHECK_PLAN_DATE, STATUS_USE, USE_SECTION, CHECK_FREQUENCY, REPORT_FORM_CODE, IS_ACTIVE, CREATED_AT)
    VALUES (
        ...
        CASE S.CHECK_FREQUENCY
            ...
            ELSE NULL
        END,
        N'Đang sử dụng', S.DEPARTMENT, S.CHECK_FREQUENCY, S.REPORT_FORM_CODE, 1, GETDATE());
