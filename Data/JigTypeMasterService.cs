...
BEGIN
    SET @DEFAULT_DRAWING_CODE = UPPER(LTRIM(RTRIM(@DEFAULT_DRAWING_CODE)));
END
ELSE IF @DRAWING_NAME IS NOT NULL AND LTRIM(RTRIM(@DRAWING_NAME)) <> ''
BEGIN
    SET @DEFAULT_DRAWING_CODE = CONCAT('DWG-', REPLACE(CONVERT(VARCHAR(19), GETDATE(), 120), '-', ''));
    SET @DEFAULT_DRAWING_CODE = REPLACE(REPLACE(@DEFAULT_DRAWING_CODE, ':', ''), ' ', '');
END
